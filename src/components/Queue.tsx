...
type QueueSource = "priority" | "context" | "autoplay" | "active";
...
const QueueItemContent = ({
  ...
  const imageUrl = song?.albums?.image_path
    ? supabase.storage.from("images").getPublicUrl(song.albums.image_path).data
        .publicUrl
  ...
  return (
    <div
      className={`group grid grid-cols-[20px_40px_1fr_20px_30px] md:grid-cols-[30px_40px_1fr_30px_30px] gap-x-2 md:gap-x-2 items-center w-full px-2 py-2 rounded-md transition-colors cursor-pointer select-none relative z-10 ${isActive ? "bg-white/10" : "hover:bg-white/5 bg-black"}`}
    >
      <div
        onClick={(e) => {
          e.stopPropagation();
      ...
      >
        {isActive && isPlaying ? (
          ...
          <span
            className={`group-hover:hidden ${isActive ? "text-green-500" : ""}`}
          >
            {source === "autoplay" ? (
              <BsStars size={12} className="text-emerald-500" />
            ) : (
              idx
            )}
          </span>
      ...
      <div className="flex flex-col min-w-0 overflow-hidden">
        <p
          className={`truncate text-sm font-medium ${isActive ? "text-green-500" : source === "autoplay" ? "text-emerald-100" : "text-white"}`}
        >
          {song?.title || "Loading..."}
        </p>
      ...
      <div className="flex justify-center items-center">
        {onHandlePointerDown ? (
          <div
            ...
            onClick={(e) => e.stopPropagation()}
            className="flex justify-center items-center text-neutral-400 hover:text-white transition p-1 cursor-grab active:cursor-grabbing z-50 touch-none select-none"
          >
...
interface DraggableQueueItemProps {
  ...
  song?: SongData;
  idx: number;
  source: "priority" | "context" | "autoplay";
  isMobile: boolean;
  onPlay: () => void;
  onRemove?: () => void;
...
const DraggableQueueItem = ({
  ...
  const handleHandlePointerDown = useCallback(
    ...
    [isMobile, clearPressTimer, startReorder],
  );
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="absolute right-4 top-0 bottom-0 flex items-center justify-center text-red-500 z-0"
      >
        <BsTrash size={20} />
      </motion.div>
      <motion.div
        style={{ opacity: addOpacity }}
        className="absolute left-4 top-0 bottom-0 flex items-center justify-center text-green-500 z-0"
      >
        <BsPlusLg size={20} />
      </motion.div>
...
export default function Queue() {
  const { isOpen, onClose, width, setWidth, activeView, toggle } =
    useQueueStore();
  ...
  // 🟢 Hook to determine mobile rendering size safely
  ...
  const currentIndex = useMemo(() => {
    const refId = player.isPlayingPriority
      ? player.lastActiveContextId
      : player.activeId;
    if (!refId) return -1;
  ...
  useEffect(() => {
    ...
    const fetchBatch = async () => {
      const { data } = await supabase
        .from("songs")
        .select(
          "id, title, duration_seconds, albums(title, image_path, artists(name))",
        )
        .in("id", toFetch);
      if (data) {
        setSongsCache((prev) => {
  ...
  const handlePriorityReorder = useCallback(
    (items: typeof priorityList) => player.setBucketB(items),
    [player],
  );
  ...
  const handleAutoplayReorder = useCallback(
  ...
  const handleRemoveFromAutoplay = useCallback(
    (uid: string) => {
      const idToRemove = uid.replace("auto-", "");
      if (player.isPlayingAutoplay && player.activeId) {
        const currentIdx = player.autoplay.indexOf(player.activeId);
        const history = player.autoplay.slice(0, currentIdx + 1);
        const future = player.autoplay
          .slice(currentIdx + 1)
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ width: 0 }}
          // 🟢 Sleeker easing curve for a native feel
          transition={{ ease: [0.32, 0.72, 0, 1], duration: 0.35 }}
          // 🟢 Anchored to the right side so it expands naturally
        ...
        >
          {/* 🟢 The inner wrapper keeps a strict width so text never squishes! */}
          <div
          ...
          >
            <div
              ...
              className={`hidden md:block absolute left-0 top-0 bottom-0 w-1 z-50 cursor-col-resize transition-colors bg-transparent hover:bg-purple-500/50 ${resizing.current ? "bg-purple-500" : ""}`}
            />
                            ...
                            {priorityList.map((item, i) => (
                              <DraggableQueueItem
                                ...
                                onPlay={() => {
                                  ...
                                  if (realIdx !== -1)
                                    player.playQueueItem(realIdx);
                                }}
                                onRemove={() =>
                                  player.removeFromPriority(item.uid)
                                }
                              />
                            ))}
                          </Reorder.Group>
                          ...
                          >
                            {contextItems.map((item, i) => (
                              <DraggableQueueItem
                                ...
                                onRemove={() =>
                                  player.removeFromContext(item.id)
                                }
                                onAddToPriority={() =>
                                  player.addToQueue(item.id)
                                }
                              />
                            ))}
                          </Reorder.Group>
                          ...
                          >
                            {autoplayItems.map((item, i) => (
                              <DraggableQueueItem
                                ...
                                onRemove={() =>
                                  handleRemoveFromAutoplay(item.uid)
                                }
                                onAddToPriority={() =>
                                  player.addToQueue(item.id)
                                }
